...
exports.fail = function (
    ...
    array_response.userMessage = handlebars_message(objectDetails)
...
exports.getAsURL = function (req, index, defaultvalue, required) {
    ...
    try {
        ...
        throw new APIError({
            ...
            templateUserMessage: `The parameter${index} URL is invalid, perhaps missing https:// at start?  E.g. use https://coinmode.com instead of coinmode.com`,
            internalDetails: {},
        })
    }
...
exports.getAsArray = function (req, index, required, max) {
    ...
    if (!Array.isArray(object)) {
        throw new APIError({
            ...
            templateUserMessage: `The parameter '${index}' must be submitted as an array`,
            internalDetails: {},
        })
    }
    if (required && object && object.length === 0) {
        throw new APIError({
            errorCode: 'INVALID_PARAM',
            objectDetails: { object },
            templateUserMessage: `The parameter '${index}' is required. It is currently empty array`,
            internalDetails: {},
        })
    }
    if (max && object && object.length > max) {
        throw new APIError({
            errorCode: 'INVALID_PARAM',
            objectDetails: { object },
            templateUserMessage: `The parameter '${index}' is capped to ${max} items. It has currently ${object.length} items`,
            internalDetails: {},
        })
    }
    return object
...
/**
 * It will make sure that `epoch_from`/`epoch_until`
 * are within reasonable timeframe specified in terms
 * of days.
 */
exports.ensureReasonableTimeframe = function ({
    epoch_from,
    epoch_until,
    asDays,
}) {
    const from = moment.unix(epoch_from)
    const until = moment.unix(epoch_until)
    const duration = moment.duration(until.diff(from))
    if (duration.asDays() > asDays) {
        /**
         * It is longer than specified timeframe so lets tell
         * the user to lower the time duration.
         */
        throw new APIError({
            errorCode: 'INVALID_PARAM',
            objectDetails: {
                epoch_from,
                epoch_until,
                currently: duration.asDays(),
            },
            templateUserMessage: [
                `The ${epoch_from} and ${epoch_until} must have a timeframe of < ${asDays} days.`,
                `Currently it is ${duration.asDays()} days.`,
            ].join(' '),
            internalDetails: {},
        })
    }
