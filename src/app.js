...
class App {
    ...
    startListening() {
        this.app.listen(this.port, () => {
            log.notify(
                false,
                null,
                `Palindrome is listening at port ${this.port}`
            )
        })
    }
    ...
    /**
     * To solve the issue of most of the code is looking for
     * `body` parameters which works for `POST` calls. `GET`
     * calls use `query`. This under the hood
     * attempts to populate `body` with the `query`
     * of `GET` calls.
     */
    populateBodyForGetCalls() {
        this.app.use((req, res, next) => {
            const { body, method, query } = req
            /**
             * Lets make sure we only do this for `GET` calls
             * and when `body` is empty
             */
            if (!Object.keys(body).length && method === 'GET') {
                req.body = query
            }
            next()
        })
    ...
    requiredAPIs() {
        ...
        this.app.get('/docs', (req, res) => res.render('/docs/index.html'))
